...
Description: In this file I implemented my swap function.
*/
...
void DoublyLinkedList<T>::swap(int i, int j){
  Node<T>* ptri = getPointerTo(i);
  Node<T>* ptrj = getPointerTo(j);
  if (ptri == nullptr || ptrj == nullptr){
    return;
  }
  T temp = ptri -> getItem();
  ptri -> setItem(ptrj -> getItem());
  ptrj -> setItem(temp);
}

//  Tests swap when there's only 2 elements 
//  Tests swap with 1st and last element when there's more than 2 items 
//  Tests swap with 2 middle elements that not are successive (not next to each other) 
